...
//
//This header is the Boxer-facing interface and deliberately includes NO DOSBox headers: all
//DOSBox types it mentions are forward-declared or defined locally, so that includers
//(BXEmulatedPrinter, BXEmulatedKeyboard, BXKeyBuffer, BXCoalfaceAudio, ...) are not exposed
//to DOSBox internals. The DOSBox-facing twin lives at DOSBox-Staging/include/BXCoalface.h,
//and the 0.83-style GFX_* entry points are defined as real forwarding functions in
//BXCoalface.mm rather than via macro remapping.
...
//Local definitions matching DOSBox's misc/types.h, so that declarations here do not
//depend on DOSBox headers. Duplicate typedefs of identical types are legal C++.
using Bit8u  = uint8_t;
using Bit16u = uint16_t;
using Bit32s = int32_t;
using Bitu   = uintptr_t;

//DOSBox 0.83's callback type, guarded with the same GFX_CALLBACK_DEFINED macro that
//misc/video.h and gui/private/common.h use, so whichever header is seen first wins.
...
#pragma mark - Input
    ...
    /// Reports the current emulated mouse resolution (in DOS pixels) from
    /// DOSBox's shared mouse state, so that Boxer's mouse bridge does not
    /// need to include DOSBox's private input headers.
    void boxer_mouseScreenResolution(int32_t *outResolutionX, int32_t *outResolutionY);

    /// Bridges for DOSBox's mouse event API (which lives in private input
    /// headers), so BXEmulatedMouse only ever talks to the coalface.
    void boxer_mouseNotifyWindowActive(bool windowActive);
    void boxer_mouseUpdateGFX(void);
